// static/js/retirement.js
// This script handles the dynamic display, editing, and calculation of
// USAF Retirement Pay data, along with yearly and running totals.
...
async function handleAmountChange(inputElement, id, year, amountType) {
    ...
    const netPayCell = document.getElementById(`USAF_Monthly_Net_Income_${monthEntry.month_name.substring(0, 3)}_${monthEntry.year}`);
    if (netPayCell) {
    ...
    try {
        const response = await fetch('/personal/api/usaf_retirement/update', { // Updated API endpoint
            method: 'POST',
            headers: {
...
function toggleYearVisibility(year) {
    ...
    const rowsToToggle = document.querySelectorAll(`#main-table-body tr[data-year-data="${year}"]`);
...
function updateRunningTotals() {
    ...
    document.getElementById('running-gross-total').textContent = formatCurrency(overallCurrentCalendarYearGrossTotal);
    document.getElementById('running-taxed-total').textContent = formatCurrency(overallCurrentCalendarYearTaxedTotal);
    document.getElementById('running-net-total').textContent = formatCurrency(overallCurrentCalendarYearNetTotal);
}
...
function updateYearlyTotals(year) {
    ...
    const yearTotalGrossCell = document.getElementById(`USAF_Yearly_Gross_Income_${year}`);
    const yearTotalTaxedCell = document.getElementById(`USAF_Yearly_Taxed_Amount_${year}`);
    const yearTotalNetCell = document.getElementById(`USAF_Yearly_Net_Income_${year}`);
...
function updateMonthlyData() {
    const today = new Date();
    const currentYear = today.getFullYear();
    // const currentMonthIndex = today.getMonth(); // No longer needed for visibility logic
    // const currentDay = today.getDate(); // No longer needed for visibility logic
    ...
    document.getElementById('current-calendar-year').textContent = currentYear;
    ...
    for (const year of sortedYears) {
        ...
        yearData.forEach((data) => {
            // isVisibleByDate logic removed - all data is always visible
            ...
            grossPayInput.id = `USAF_Monthly_Gross_Income_${data.month_name.substring(0, 3)}_${data.year}`;
            ...
            taxedAmountInput.id = `USAF_Monthly_Taxed_Amount_${data.month_name.substring(0, 3)}_${data.year}`;
            ...
            netPayCell.id = `USAF_Monthly_Net_Income_${data.month_name.substring(0, 3)}_${data.year}`;
        ...
        yearTotalRow.id = `USAF_Yearly_Total_Row_${year}`;
        ...
        yearTotalRow.innerHTML = `
            ...
            <td id="USAF_Yearly_Gross_Income_${year}">${formatCurrency(0)}</td>
            <td id="USAF_Yearly_Taxed_Amount_${year}">${formatCurrency(0)}</td>
            <td id="USAF_Yearly_Net_Income_${year}" class="rounded-br-lg">${formatCurrency(0)}</td>
        `;
...
function exportData() {
    ...
    document.getElementById('export-output-area').classList.remove('hidden'); // Show the export area
...
async function initializeApp() {
    try {
        // Fetch data from Flask API endpoint for USAF Retirement
        const response = await fetch('/personal/api/usaf_retirement/data');
        if (!response.ok) {
        ...
        // Attempt to render even if data fetch fails (will be empty or stale)
        // updateMonthlyData(); // No need to call this here, as it will be called by window.onload
    }

    // Attach event listeners after initial render
    document.getElementById('export-button').addEventListener('click', exportData);
    document.getElementById('copy-export-button').addEventListener('click', () => copyToClipboard('exported-data-textarea'));
}
